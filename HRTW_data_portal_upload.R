# HRTW (Human Right to Water) - https://www.waterboards.ca.gov/water_issues/programs/hr2w/index.html
...
# download the most recent dataset #----------------------------------------------------------------------------------------#
    ...
    # in the new directory, download the original (unmodified) files
        # active
            ...
            download.file(url = path_active, destfile = paste0(current_year_month, '//', file_active, extension), method = 'curl')
        # rtc
            path_rtc <- paste0(base_url, file_rtc, extension)
            download.file(url = path_rtc, destfile = paste0(current_year_month, '//', file_rtc, extension), method = 'curl')
    # read the datsets into R
        data_active <-  read_excel(path = paste0(current_year_month, '//' , file_active, extension))
        data_rtc <-  read_excel(path = paste0(current_year_month, '//' , file_rtc, extension))
...
# write out the revised datasets as .csv files #----------------------------------------------------------------------------#
    # Active
        out_file_active <- paste0(current_year_month, '//', file_active, '.csv')
        write_csv(x = data_active, path = out_file_active, na = 'NaN')
    # RTC
        out_file_rtc <- paste0(current_year_month, '//', file_rtc, '.csv')
        write_csv(x = data_rtc, path = out_file_rtc, na = 'NaN')
