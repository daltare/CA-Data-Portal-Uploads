...
geotracker_link <- glue('https://geotracker.waterboards.ca.gov/data_download/geo_by_county/{county}GeoXY.zip') # see: https://geotracker.waterboards.ca.gov/data_download/geo_by_county/

# define data portal resource ID
resourceID <- 'c451b186-d8a5-424f-b886-37e4d957aeff' # https://data.ca.gov/dataset/ground-water-water-quality-regulatory-information/resource/c451b186-d8a5-424f-b886-37e4d957aeff

# build the filename
filename_geotracker <- glue('{county}GeoXY')
...
temp <- tempfile()
download.file(url = geotracker_link, destfile = temp, method = 'curl')
data_geotracker <- read_tsv(unz(temp, glue('{filename_geotracker}.txt')), 
...
fields_numeric <- c('LATITUDE', 'LONGITUDE', 'XY_ACC_VAL')
...
out_file <- here(glue('{filename_geotracker}_{Sys.Date()}.csv'))
