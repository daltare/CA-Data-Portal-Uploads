...
# download the files and read data into R
    ...
    df_data <- readr::read_csv(file = temp, guess_max = 999999) #, quote = '') #%>% select(-X264)
...
# check dataset for portal compatibility and adjust as needed
    ...
    # date fields - convert dates into a timestamp field that can be read by the portal
        fields_dates <- c('priority_date', 'receipt_date', 'rejection_date', 
                          ...
                          'effective_to_date', 'effective_date', 'update_datetime',
                          'pod_last_update_date','permit_original_issue_date', 'complete_construction_date',
                          ...
                          'pet_last_update_date', 'enf_case_start_date', 'enf_case_closure_date')
         for (counter in seq(length(fields_dates))) {
            # convert the date field to ISO format
                dates_iso <- mdy_hms(df_data_filter[[fields_dates[counter]]])
...
# write out the revised dataset as a .csv file
    out_file <- paste0('water_rights_list_', Sys.Date(), '.csv')
