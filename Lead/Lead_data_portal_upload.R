...
# ENTER THESE VARIABLES #####################################################################################################
    filename <- 'monthly_posting_final'
    extension <- '.xlsx'
    ...
    file_save_location <- 'C:\\David\\_CA_data_portal\\Lead\\'
...
# download the most recent dataset #----------------------------------------------------------------------------------------#
    ...
    # in the new directory, download the original (unmodified) file
        ...
        download.file(url = path, destfile = paste0(file_save_location, current_year_month, '//', filename, extension), method = 'curl')
    # read the datset into R
        data_lead <-  read_excel(path = paste0(file_save_location, current_year_month, '//' , filename, extension))
...
# write out the revised dataset as a .csv file #----------------------------------------------------------------------------#
    out_file <- paste0(file_save_location, current_year_month, '\\', filename, '.csv')
    write_csv(x = data_lead, file = out_file, na = 'NaN')
