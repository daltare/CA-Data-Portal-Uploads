...
The only required step is to run the `water-rights-master-flat-file.R` script (e.g., run `source('water-rights-master-flat-file.R')` or open the file in RStudio and click the `Source` button). This should handle all parts of the update process.

To automatically run this update process on a set schedule on a Windows computer, you can create a task in "Task Scheduler" that runs the `Call_WaterRights-Master.bat` file on a specified day and time (the `Call_WaterRights-Master.bat` file is in the `_Call-Scripts` directory at the top level of this repository).

If the process fails at any point, it is set up to send an automated email alerting you that the process has failed (and an attempt at telling you why it failed).
