# This script downloads data from CEDEN, and formats it for posting to the data.ca.gov
# (open data) portal
...
# 4 - get / format data -------------------------------------------------------
## 4.1 - setup CEDEN database connection ----
...
for (data_type in names(tables_list)) {
    ...
    # data_type <- 'chemistry' # for testing
    # 'toxicity' 'benthic' 'chemistry' 'tissue' 'habitat'
    
    ## 4.2 - get file & table name for given data type ----
    ...
    ## 4.3 - create a temporary directory/file to store combined data (across all years -- gets converted to zip file)
    temp_dir <- tempdir()
    ...
    for (get_year in years_list) {
        ...
        ## 5.2 - convert date/numeric data types (based on data dictionaries) ----
        ...
        ## 5.3 - format text fields ----
        ### ensure all text records are in UTF-8 format, convert if not ----
        {
            # tic()
            df_query_result <- df_query_result %>% 
        ...
        ## 5.4 - re-order columns ----
        fields_all <- df_types %>% 
            pull(column)
        ## remove the data quality fields from the list, since they haven't been added to the dataset yet
        fields_all <- fields_all[!fields_all %in% c('DataQuality', 'DataQualityIndicator', 'Datum')]
        df_query_result <- df_query_result %>% 
        ...
        # 8 - format data for portal compatibility -----------------------------------
        ## 8.1 - format date fields ----
        gc()
        # convert dates and times into a timestamp field that can be read by the portal
        ## convert dates to text, and for NAs store as '' (empty text string) - this converts to 'null' in Postgres
        for (col in fields_dates) {
            df_query_result <- df_query_result %>%
                mutate("{col}" := as.character(df_query_result[[col]]))
            df_query_result <- df_query_result %>% 
                mutate("{col}" := replace_na(df_query_result[[col]], ''))
        }
        # check
        # sum(is.na(df_query_result$sampling_date))
        # sum(df_query_result$sampling_date == '', na.rm = TRUE)
        # sum(is.na(df_query_result$analysis_date))
        # sum(df_query_result$analysis_date == '', na.rm = TRUE)
        gc()
        
        
        ## 8.2 - format numeric fields ----
        gc()
        ...
        for (col in fields_numeric) {
            df_query_result <- df_query_result %>%
                mutate("{col}" := as.character(df_query_result[[col]]))
            df_query_result <- df_query_result %>% 
                mutate("{col}" := replace_na(df_query_result[[col]], 'NaN'))
        }
        # check
        # sum(is.na(df_query_result$result))
        # sum(df_query_result$result == 'NaN', na.rm = TRUE)
        # sum(is.na(df_query_result$mdl))
        # sum(df_query_result$mdl == 'NaN', na.rm = TRUE)
        
        
        # 9 - write output csv files --------------------------------------------------
        gc()
    ...
    # 9 - convert combined data file to zip file ----------------------------------
    ...
    # 10 - for toxicity and benthic data, save the raw combined file
